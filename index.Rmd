...
This document explains how to use R, subject to some caveats: starting in chapter \@ref(tables), I only cover the packages I use; chapter \@ref(work) only covers the analytical tools I use; and, to keep it short, I have skipped discussion of statistical methodology and pretty graphs. 
...
The comment symbol, `#`, appears in the results box so that you can safely copy-paste them into the console as well (when dragging over multiple code blocks). Unfortunately, R does not have any syntax for block comments.
