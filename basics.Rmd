...
```{block2 na-classes, type='rmd-caution'}
...
```{block2 fancy-splicing, type='rmd-details'} **Special slices.** 
...
**Arithmetic with logical values.** Particularly when exploring data, we often want to tally observations meeting some condition. To do this in R, we can treat true/false like 1/0: `is_gt = c(1,5,2) > c(1,2,3); sum(is_gt)`. Tallying will be revisited in \@ref(tallying).
```
...
```{block2, type='rmd-details'} **Managing packages.** 
With `installed.packages`, dependencies and version numbers can be viewed. Some other tools, [rocker](https://github.com/rocker-org/rocker) and [drat](https://github.com/eddelbuettel/drat), also look promising.
```
