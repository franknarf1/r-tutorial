...
The Examples section always usefully illustrates functionality; you can run it in your own console like `example("LETTERS")`. Additional illustrations are available using `demo`; type `demo()` for a listing.

Try typing `help.start()`. You should see links to documents including "An Introduction to R" and "The R language definition". These are the essential reference documents on R, also [available in pdf online](https://cran.r-project.org/manuals.html).
...
From there, you can subassign iteratively if necessary: `x[1] <- "A"`, `x[2] <- "B"`, and so on.
...
todo -- setops has been removed; maybe migrate to work.rmd
...
**Arithmetic with logical values.** Particularly when exploring data, we often want to tally observations meeting some condition. To do this in R, we can treat true/false like 1/0: `is_gt = c(1,5,2) > c(1,2,3); sum(is_gt)`. Tallying will be revisited in @ref(tallying). todo -- #tallying was removed, maybe move elsewhere
```
...
Additional convenience functions for matrix algebra are covered in @ref(matrix-more). todo -- figure out where this @matrix-more should go
...
The arguments are being passed out-of-order, but this still works since the arguments are being passed by name. Passing arguments by the position, like `sort(x, TRUE)`, is a riskier proposition, since it is harder to get right initially and to interpret correctly later. Some data.table idioms for merging (\@ref(dt-merging)) involve passing arguments out-of-order.
...
If you run into a function and don't know what package it is from, use `help.search` or its `??` shortcut, like `??fortunes`. 
...
Many such hidden functions are "methods," discussed further in @ref(methods). todo -- there is no #methods section to refer to -- There are also several functions that *should* be hidden, documented at `?.subset`, `?.getNamespace` and elsewhere. These docs always have prominent notes like:
...
details -- masking and reserved words. it's okay to "mask" a function or object that exists. it's just not a good idea if it's going to confuse you or others later. for example, don't name a matrix "matrix", since that's the name of the constructor function
