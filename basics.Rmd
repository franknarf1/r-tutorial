...
```{r todo-more-vec-ex, eval=FALSE, echo=FALSE}
something to do with assigning names
...
```{r todo matrix-ex, eval=FALSE, echo=FALSE}
rbind/cbind two matrices
overwrite a submatrix with another matrix
```
...
4. Read `?pmin`, then write a function that takes three vectors and computes $\min((x-y)^+, z)$ elementwise, where${()}^+$ denotes [the positive part](http://mathworld.wolfram.com/PositivePart.html), testing on `x = c(10,20,30); y = 15; z = c(25,15,5)`.
