...
Try typing `help.start()`. This will open a page of links to manuals including "An Introduction to R", "The R Language Definition" and the "R FAQ" (under "Resources"). These are also [available in pdf online](https://cran.r-project.org/manuals.html).
...
Sometimes, we need closer inspection for debugging. The functions `unclass` (which removes the class) and `dput` (which prints R code to create the object) are helpful for this. See \@ref(erroneous-na) for an example relevant to date and time objects.
...
```{r setnames-base-bad, error = TRUE}
nms = c("a", "b")
c(nms[1] = 1, nms[2] = 2) # error
```
```{r setnames-base, echo=-1}
nms = c("a", "b")
...
**Recycling.** What would be the natural behavior of `x[TRUE]` in the example above? Well, in R, it acts like `x[c(TRUE, TRUE, TRUE, TRUE)]`, without giving any warning. This is called recycling -- the argument `i` is repeated until it is long enough for the context it is used in. What about `x[c(TRUE, FALSE)]`? The full rules for recycling can be found in the [R intro doc](https://cran.r-project.org/doc/manuals/r-release/R-intro.html#The-recycling-rule).
```
...
### Sorting, rank, order {#sort-rank-order}
...
2. What is the length of `list(1, c(2,3,4))`?
3. If I run `x = c("a","b")` and then `x[c(2,1)] = c("c","d")`, what is the value of `x[1]`?
4. What is the command to open documenation for for the assignment procedure used in `L$X <- list(3)`?
...
### Computing with matrices {#matrix-misc}

Matrix algebra functions are covered in \@ref(matrix-algebra). Some other handy functions are:
...
1. From `x = 1:6`, build a 3x2 matrix rowwise like `m(x, _fill_this_in_)`.
2. From `m = matrix(1:6, 2)`, extract a submatrix containing the first column.
3. Read `?do.call` and from `L = list(c(1,1), c(2,2), c(3,3))`, bind the elements of `L` as columns of a single matrix.
4. From `m = matrix(1:4, 2)`, overwrite the upper left and lower right cells with `0`.
...
This is also rarely needed since `DT[i, {...}, by]` syntax for data.tables (\@ref(tables)) achieves the same result.
...
These are rarely needed and can be confusing for obvious reasons. Section \@ref(dt-subassign) will discuss a different approach.
...
**Recycling, again.** As we saw in \@ref(slicing), recycling kicks in for any vectorized function. So we have `c(-2,2) ^ c(1,2,3,4)` working, with no warning given. Fortunately, there is a warning if the recycling is imperfect, like `c(-2,2) ^ c(1,2,3)`.
```
...
The undecorated `*` will always work elementwise (\@ref(vectorization)) and silently recycle values:
...
- `%in%` to test membership (see \@ref(sets)); 
...
we see that the order of arguments here contrasts with how it was used in \@ref(initializing):

```{r preallocate-again, eval=FALSE}
x = vector(length = 5, mode = "list")
```

The arguments are being passed out-of-order, but it still works since the arguments are being passed by name. Passing arguments by the position, like `sort(x, TRUE)`, is a riskier proposition, since it is harder to get right initially and to interpret correctly later. Data.table idioms I follow for merging (\@ref(dt-joins)) all involve passing arguments out-of-order.
...
Now, the `y` the function uses is part of the "environment" of the function. Environments are essentially lists, so the value can be extracted following syntax from \@ref(extract-list):
...
1. Write `ndim`, a function to count the number of dimensions an array has.
2. Review (\@ref(seq)) and read `?cumsum`, then write a `cummean` function to compute the cumulative mean.
3. Install a new package, then use `library(package = "_your_new_package_")` to find several objects included with the package.
4. Read `?pmin`, then write a function that takes three vectors and computes $\min((x-y)^+, z)$ elementwise, where${()}^+$ denotes [the positive part](http://mathworld.wolfram.com/PositivePart.html), testing on `x = c(10,20,30); y = 15; z = c(25,15,5)`, for which the correct result is `c(0, 5, 5)`.
...
1. Write a vectorized function for the root mean squared deviation between two vectors, $\sqrt{\frac{1}{n}\sum_1^n (x_i - y_i)^2 }$ where `n` is how long the vectors are. For `x = 1:4; y = c(6,1,4,1)` it should return `3`.
...
It is useful to keep in mind that data frames are really lists of columns, not matrices. We can use `lapply` (\@ref(lapply)) to iterate over such lists:
