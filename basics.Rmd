...
2. Review (\@(seq)) and read `?cumsum`, then write a `cummean` function to compute the cumulative mean.
...
1. Write a vectorized function for the root mean squared deviation between two vectors, $\sqrt{\frac{1}{n}\sum_1^n (x_i - y_i)^2 }$ where `n` is how long the vectors are.
