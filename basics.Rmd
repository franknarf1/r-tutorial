...
Manuals and additional reference documents can be found by typing `help.start()` and clicking through to "An Introduction to R" or other documents.
...
To explore the set of loaded objects, see the tools in \@ref(tables).
...
<span class="spdetails" id="fancy-slicing">
...
We get one true/false value for each element on the left-hand side. Duplicate values on the right-hand side are ignored. That is, the right-hand side is treated like a *set*. We will look at other set operations in \@ref(set-ops).
...
All elements of a matrix must have the same class, just like a vector. So you can have a "character matrix", a "numeric matrix", etc. 
...
Matrices can also be built from vectors with `cbind` and `rbind`:
...
These functions also take matrices as arguments.
...
You can get fancy with indices, in all the ways described in \@ref(slicing). It is often covenient to leave one index blank, which means "select all"; try `m[, 2, drop = FALSE]`. 

<span class="spnote" id="drop">
...
Implicitly, matrices are thought of as having observations as rows and variables as columns. As a result, `head` and `tail` will return rows from the top and bottom of a matrix, respectively.
...
Essentially all computations involving missing values will return a missing value. One exception is `NA^0` since no matter what number the true value is, this expression will evaluate to `1`. The `min` and `max` functions can ignore missing values, as noted in their docs. \@ref(dt-summary-stats) covers other summarizing functions, all of which similarly offer to ignore `NA`s.
...
**Arithmetic with logical values.** Particularly when exploring data, we often want to tally observations meeting some condition. To do this in R, we can treat true/false like 1/0: `is_gt = c(1,5,2) > c(1,2,3); sum(is_gt)`. Tallying will be revisited in \@ref(tallying).</span>
...
**Logic with numeric values.** Numbers are treated as `FALSE` when zero and `TRUE` otherwise. This allows us to use `length(x)` as a test instead of `length(x) > 0`.</span>
...
Remember that the undecorated `*` will always work elementwise and silently recycle values.
...
The first time this is run during an R session, a prompt asking which mirror to download from will pop up.
...
This code works because it indicates the "namespace" that the function comes from. 

Another way to use functions from a package is to load it with `library`: 
...
This adds all of the package's functions and other objects to the global namespace. The downside to loading a package is the risk of "namespace conflicts": 

- If the loaded package has an object with the same name as something defined by the user, that object won't enter the global namespace.
- If two loaded packages contain objects with the same name, the last-loaded object gets precedence for entry into the global namespace.
- Some packages with elbow their way past built-in base functions when loaded. This should probably be taken as a red flag.

Fortunately, in all these cases, R will print a message about the conflicting names.

There are commands to unload packages (`unloadNamespace` and `detach`), but I always just restart my R session instead.
...
As with any community-sourced code, if the docs are bad, assume the worst about the code itself. My favorite example of this among mainstream packages is [functional](https://cran.r-project.org/web/packages/functional/index.html).
...
If a package is loaded, it cannot be updated, so close all R sessions where the package is loaded before updating it.

<span class="spdetails">
**Managing packages.** With `installed.packages`, dependencies and version numbers can be viewed. Some other tools, [rocker](https://github.com/rocker-org/rocker) and [drat](https://github.com/eddelbuettel/drat), also look promising.
</span>
...
The arguments are being passed out-of-order, but this still works since the arguments are being passed by name. Passing arguments by the position, like `sort(x, TRUE)`, is a riskier proposition, since it is harder to get right initially and to interpret correctly later.
...
`return` provides the return value. If a `return` statement is reached before the end of a function, the rest of the code is not run. If no return statement is given, the last value is used. To return multiple objects, combine them in a list and return it instead.
...
Notice that in the last section, `f` modifies its input. Does this carry over after a function call?
...
We see that it does not carry over. Usually, nothing created or altered inside a function does, meaning there are no [side effects](http://programmers.stackexchange.com/q/40297/104607). There are ways to write functions with side effects in base R (`<<-` and `assign`), but these are discouraged:
...
So R only looks for `y` when it needs it and no sooner. This is called "lazy evaluation." It has advantages, but can lead to mistakes. 
...
If you run into a function and don't know where it is from, use `help.search` or its `??` shortcut, like `??fortune`.
...
Many such hidden functions are "methods," discussed further in \@ref(methods). There are also several functions that *should* be hidden, documented at `?.subset`, `?.getNamespace` and elsewhere. These docs always have prominent notes like:
...
- We have to refer to `x` multiple times, creating overhead if we want to reuse this code elsewhere.
...
<span class="spnote">
**Dummy variables.** First, if you find yourself wanting to do `x = if (cond) 1 else 0`, don't. As mentioned in \@ref(arithmetic), the logical variable `cond` itself is perfect for storing this dummy variable. It will be treated as numeric in any arithmetic calculations. Besides, there is certainly no reason to prefer the floating-point values `0` and `1` over integers `0L` and `1L`.
</span>
...
Data frames are printed with observations on rows and variables on columns. In many ways, they behave like matrices: working with functions like `dim`, `ncol`, `nrow`, `head` and `tail`; using `DF[i, j]` syntax for slices; and even using `DF[im]` syntax for matrix indexing (which rarely makes sense).
...
The last item above is nonsensical in two ways:
...
Naturally, if the data already is entirely numeric and the remaining analytical tasks are all linear algebra, a matrix will be simpler and computationally faster to work with than a data frame.
