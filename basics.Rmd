...
```
```{r matrix-slicing-pre-print, echo=FALSE}
m
```
```{r matrix-slicing}
...
m = matrix(c(1,2,3,4,5,6), nrow = 2, dimnames = list(c("a", "b"), c("x", "y", "z")))
```
```{r matrix-subset-pre-print, echo=FALSE}
m
```
...
Finally, `drop = TRUE` can be used to select a column or row, like `m[, 2]` (where `drop = TRUE` is the default) or a single element, like `m[1, 2]` or `m["a", "y"]`.
...
- Some packages will elbow their way past built-in base functions when loaded, with a warning. This should probably be taken as a red flag.
...
**My opinion on trusting packages.** If you don't understand a package's docs, don't use it for anything serious. If the version number of the package is below 1.0, be ready to keep track of its development (since most package developers follow a pattern like [the one Hadley Wickham's](http://r-pkgs.had.co.nz/release.html)). Tools for input, output or graphing are fragile in any language, so isolate the code that depends on them as much as possible.
```
