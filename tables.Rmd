...
The official vignettes for the package are a great way to start with the package. Run `browseVignettes(package="data.table")` or visit the [wiki](http://r-datatable.com/Getting-started) to browse them. (Beware that as of March 2017, the website vignettes are somewhat out-of-date.) 

The website also includes links to other useful materials (an online course, presentation materials, blog posts). I started by reading some slides and the FAQ in full before using the package.
...
**Use column names, not numbers.** Subsetting by hard-coded column number, like `cols = 2:3; DT[, ..cols]` works but is discouraged. Column numbers can easily change in the course of writing or updating a script, invalidating column number references in a way that will be annoying to debug. See the first answer in `vignette("datatable-faq")` for a deeper discussion.
```

```{block2 dt-dfbrackafrak, type='rmd-details'}
**How data frame slicing works.** This is really getting in the weeds, since I suggest not using data frames at all, but you'll see two major differences if you do. First, `DT[, "z"]` will extract the `z` column instead of taking a slice thanks to the `drop=TRUE` default (which also came up regarding matrices in the last chapter). Second, `DT[1:2]` will slice the first two *columns* instead of the first two rows, thanks to the fact that data frames (and data.tables) are lists and the no-comma usage of `[` triggers list indexing.
```
...
There are some syntactical shortcuts for slicing to a set of columns; see \@ref(program-cols).
...
Typically, the `i` and `j` arguments are called by position, as seen here, rather than by name (like `i = `). Whenever `j` evaluates to a list, the output will be a new data.table:
...
The use of `by=` is very similar to a `for` loop (\@ref(for-loops)), but it is better in a few important ways: 

- We don't have to manually construct and keep track of some "split-up data" list to iterate over.
- Optimized by-group functions are used when available (see `?GForce`)
...
There are some syntactical shortcuts for writing `by=`; see \@ref(program-cols).

#### An example

The task for `j` can really be anything. Just as a demonstration of its power, here's how it can be used for saving per-group plots to a PDF:
...
Plotting graphs is beyond the scope of this document, but the example above should make intuitive sense after reading the docs for each object, `?MASS::birtwt`, `?pdf`, et al. For each group, we're now printing a line with `cat` and saving a plot. 
...
review http://stackoverflow.com/documentation/data.table/3785
...
So, we just write `j` of `DT[i,j,by]` as an expression to compute the summary statistic.
...
So, we just write the relevant columns in `.SDcols` (which does not yet support `.()` notation) and refer to `.SD`, the ***S**ubset of **D**ata*. Within each `by=` group, the query has access to `.SD` with rows for that group and columns as specified in `.SDcols`.

We can use `lapply` to iterate over columns since `.SD` is a data.table, which is a list of column vectors under the hood (as seen before in \@ref(lapply-df)). The column names carry over to the result because `lapply` always carries over names. 

While `.()` is not yet supported, there are a variety of convenience features for specifying `.SDcols`, covered in \@ref(program-cols).

refer to http://stackoverflow.com/documentation/data.table/3787
...
todo -- this section does not relate to modification and should be moved -- 
...
**`:=` is the function both for creation *and* modification of columns.** This will be covered it more detail in subsequent sections, but is worth emphasising. In particular, this contrasts with Stata (which uses distinct verbs `gen`, `egen` and `replace`).
```
...
todo warning similar to vignette about chaining. vignette says: "What is the difference between `flights[hour == 24L, hour := 0L]` and `flights[hour == 24L][, hour := 0L]`?"

### Other by-reference modifications
...
setkey (explained later with link)

setattr more generally (illustrate with factor levels)
...
todo -- note about functions altering input by reference, covered in the ":= for its side effect" section of the vignette. also perhaps mention shallow()

also warn about names

### Exercises

todo -- use carsDT = data.table(mtcars, rn = TRUE), overwrite am with factor "automatic" and "manual"
...
If your data is well organized, this will be the join you need most of the time. It modifies one table, `x`, by
