...
[The official vignettes](https://github.com/Rdatatable/data.table/wiki/Getting-started) for the package are a great way to start with the package. I started by reading some slides and the FAQ in full before using the package. Further introductory materials are offered when the package is loaded:

-- todo watch SO for a resolution to this issue of non-printing msgs

```{r dt-startup, echo=1}
library(data.table)
my_attach = data.table:::.onAttach
body(my_attach) <- body(my_attach)[[2]][[3]]
my_attach()
```
...
refer to http://stackoverflow.com/documentation/data.table/3781
...
refer to http://stackoverflow.com/documentation/data.table/4976
...
### Handling unmatched levels with `CJ`

-- argh, I wrote a bit about self-joins here but apparently mis-git it. will have to see if it can be recovered
...
### Rolling and non-equi joins

leave the efficiency bit to the next chapter (on sequences > rolling computations)
...
When joining on a single character column, the `.()` in `i` can be skipped.
