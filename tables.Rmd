...
The official vignettes for the package are a great way to start with the package. Run `browseVignettes(package="data.table")` or visit the [wiki](https://github.com/Rdatatable/data.table/wiki/Getting-started) to brows them. The website also includes links to other useful materials (an online course, presentation materials, blog posts).

I started by reading some slides and the FAQ in full before using the package.
...
todo -- move this block to wherever .() is first used --
...
It is sometimes useful to match on a range of values. To do this, we explicitly name all columns in `i` and define inequalities in `on=`. Suppose we want to see, for every time `t`, how many individuals were seen in the previous three days:
...
DT
tg = DT[, seq.int(min(t), max(t))] # range of dates of interest
