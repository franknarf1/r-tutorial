...
This can also be done by group, like

```{r mdesc-group, eval=FALSE}
airDT[, .(
  col = names(.SD),
  nna = nna <- colSums(is.na(.SD)),
  nok = .N - nna,
...
The `as.list` is necessary to split the results over multiple columns. It is inefficient, but that won't matter much when exploring data, I imagine.
...
We can plot a binned version of the variable; or instead a histogram; or a kernel approximation to the density; or a box plot:
...
Note that the `table` approach includes zeros, while they don't show up in the aggregation using `.N`.
...
Some good references are the Wikipedia pages on [normalisation](https://en.wikipedia.org/wiki/Database_normalization) and the [relational model](https://en.wikipedia.org/wiki/Relational_model); and [Hadley Wickham's exposition aimed at R users](https://www.jstatsoft.org/article/view/v059i10).
...
1. Each table has keys that we can characterize and test. To verify that there are no duplicates, for example, we can do
    ...
    `do.call` and other helpful programming tricks are covered in \@ref().
...
my_tuples %>% lapply(do.call, what = CJ)
```
...
Uglier but quicker (in terms of programming time) is to use `get`:
...
The data.table package has many global options related to optimisation and console output. To review them...
