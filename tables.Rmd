...
-- todo insert more links to official vignettes --
...
The name is a pun on [René Magritte's pipe painting](https://en.wikipedia.org/wiki/The_Treachery_of_Images).
...
todo -- note CEDTA or generally safety of using data.table where data frame is expected
...
#### Powerful syntax {#dt-syntax-better}
...
**Optimized data.table calls.** The last call is optimized in a couple ways. First, the inequality in `i` triggers creation of an "index" in terms of `x` (or [will do](https://github.com/Rdatatable/data.table/issues/1068)), making subsequent indexing on `x` faster thanks to binary search, documented at `?indices`. Second, the `sum` in `j` is computed using an optimized "grouped sum" or `gsum` computation, documented at `?GForce`.
```
...
The task for `j` can really be anything. Just as a demonstration of this power, here's how it can be used for saving per-group plots to a PDF:
...
This is better than a `for` loop (\@ref(for-loops)), since we don't have to manually construct some "split-up data" to iterate over; don't have to worry about intermediate variables like `case` contaminating the global environment; and don't have to define what we're doing as a function of prespecified variables -- we can just use any columns of the data.table.
...
this section just reviews how to take actions familiar from the last chapter

### Slicing

DT[i, j, by] is the primary syntax, but for consistency with data frames, DT[i, j] is avaiable for taking slices.

e.g., mtcars[1:2, c("mpg", "cyl")] drop=FALSE is *always* enforced

when using a normal j statement... `mtcars[1:2, .(mpg, cyl)]`

if you want to characterize a row slice for some reason (debugging?), use j=.N or which=TRUE
...
DT[[colname]] for the full vector

DT[i, colname] just a special case of evaluating `j`


### Inspecting

Consider the `mtcars` data set on cars:

```{r agg-pre}
carsDT = data.table(mtcars, keep.rownames = TRUE)
```

As seen in (\@ref(View)),  that we can view the full data set in a new window with `View`. Alternately, we can print the whole table to the console with `print` and `nrow=Inf`:

```{r dt-view, eval=FALSE}
carsDT %>% print(nrow = Inf)
...
str -- extra attributes (indices, key), pointer (ref save/load)
...
This data set has two categorical variables: `am` for automatic (0) or manual (1) transmission; and `vs` for v (0) or straight (1) engine shape. Suppose we want to compare the mean horsepower, `hp`, across these categories:
