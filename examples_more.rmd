...
# capture output of system/system2 (credit Uwe)

see their docs

fread can also do this. linux machines example: `fread("top -a -b -n 1", check.names = TRUE)`

# read/write to file (cr abhiieor)

file(), read/writeLines, cat

# get user input (cr ashish)

readline

motivating example: swirl pkg

# reading and writing data frames (meta topic)

binary (rds), plain text (delimited, fixed width), foreign, relational DB

# reading and writing other data

graphs, geo data (raster or vector), whatever
...
difftime for IDate, ITime pairs
...
plain text with sink and options(width=)

delimited or fixed width with fwrite, write*

knitr::kable, stargazer, tables::tabular, texreg, xtable, DT/shiny/whatever
